...
class AddPermitVehicleEditFragment:  BaseFragment() , ListnerForDialog {
    ...
    override fun onCreateView(
        ...
        view.tv_permit.text=getString(R.string.edit_permit)
        view.tv_photo_permit.text=getString(R.string.edit_permit_photo)
    ...
    private fun uploadingUserPhoto() {
        ...
        mCompositeDisposable_update_Photo?.add(
            ApiRequestClient.createREtrofitInstance()
                .editImageForVehiclePermitt(  AppConstants.VEHICLEDETAILID,mPermitNumber, mPermitValidDate,"true")
    ...
    private fun handleResponse(response: ResponseFromServerVehicleDetailList) {
        ...
        if(response.vehicle_permit.size>0) {

            et_permit_number.setText(response.vehicle_permit[0].permit_no)
            et_valid_upto.setText(response.vehicle_permit[0].permit_valid)
            Glide.with(activity!!).load(response.vehicle_permit[0].permit_photo).into(iv_profile)
        }
    ...
    private fun editingUserPhoto() {
        ...
        mCompositeDisposable_update_Photo?.add(
            ApiRequestClient.createREtrofitInstance()
                .editImageVehiclePermit(body, id)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
