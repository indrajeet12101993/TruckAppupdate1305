...
import kotlinx.android.synthetic.main.fragment_add_driver_adress.view.*
...
class AddDriverAdressEditFragment : BaseFragment() , ListnerForDialog {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.fragment_add_driver_adress, container, false)
        iv_profile=view.iv_profile
        dataManager = BaseApplication.baseApplicationInstance.getdatamanger()
        view.tv_attach.text=getString(R.string.edit_current_adress_photo)
    ...
    private fun uploadingUserPhoto() {
        ...
        val body: MultipartBody.Part = MultipartBody.Part.createFormData("address_photo", file!!.name.toString(), reqFile)
        ...
        mCompositeDisposable_update_Photo?.add(
            ApiRequestClient.createREtrofitInstance()
                .editImageForAdressPhoto(body, id)
    ...
    private fun handleResponse(response: ResponseFromServerDriverDetails) {
        ...
        Glide.with(this).load(response.current_address_photo[0].address_photo).into(iv_profile)
