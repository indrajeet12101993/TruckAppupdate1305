...
import kotlinx.android.synthetic.main.fragment_add_vehicle_puc.view.*
...
class AddVehiclePucEditFragment:  BaseFragment() , ListnerForDialog {
    ...
    override fun onCreateView(
        ...
        val view: View =inflater.inflate(R.layout.fragment_add_vehicle_puc, container, false)
        view.tv_add_puc.text=getString(R.string.edit_puc)
        view.tv_addphoto_puc.text=getString(R.string.edit_puc_photo)
    ...
    private fun uploadingUserPhoto() {
        ...
        mCompositeDisposable_update_Photo?.add(
            ApiRequestClient.createREtrofitInstance()
                .uploadImageForVehiclePuc(AppConstants.VEHICLEDETAILID ,mPermitValidDate,"true")
    ...
    private fun handleResponse(response: ResponseFromServerVehicleDetailList) {
        ...
        if(response.vehicle_puc.size>0) {


            et_valid_upto.setText(response.vehicle_puc[0].puc_valid)
            Glide.with(activity!!).load(response.vehicle_puc[0].puc_photo).into(iv_profile)
        }
